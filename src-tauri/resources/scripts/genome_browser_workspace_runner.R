...
progress <- function(value, detail) message(sprintf("[genome-browser-workspace][%s%%] %s", value, detail))
progress(8, "Validating transcript workspace inputs")
...
progress(20, "Loading transcript annotations")
...
progress(62, "Reading exon sequences from the reference FASTA")
...
progress(78, "Writing transcript FASTA, index and BED interval files")
...
progress(90, "Deriving transcript UTR and CDS regions")
if (length(cds_tx)) {
...
progress(100, "Finalizing transcript IGV workspace")
